...
Usage (from hexo-theory/; Modal already authenticated as 'sub-surface'):
    ...
    modal run cloud/modal/modal_bakeoff.py::smoke_test
        # ~$0.01: one pairing, 2 openings, verifies image + arena import.

    modal run cloud/modal/modal_bakeoff.py::screen --openings 25 --budget-s 1.0
    ...
    modal run cloud/modal/modal_bakeoff.py::screen --openings 100 --bots fast_tactical,fork_aware_d1.2,heuristic_d1.1
...
def _run(bot_names: list[str], openings: int, budget_s: float,
    ...
    summary = {"bots": bot_names, "openings": openings, "budget_s": budget_s,
               ...
               "pairings": table}
    out_path = Path(out) if out else RESULTS_ROOT / "modal_bakeoff_screen.json"
...
def smoke_test():
    ...
    _run(["fast_tactical", "heuristic_d1.1"], openings=2, budget_s=1.0,
         ...
         out=str(RESULTS_ROOT / "modal_bakeoff_smoke.json"),

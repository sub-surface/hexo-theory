...
Usage (run from hexo-theory/; Modal already authenticated as 'sub-surface'):
    ...
    modal run cloud/modal/modal_app.py::smoke_test
    ...
    modal run cloud/modal/modal_app.py::corpus --backend rust --n-games 20000 --sims 64 --max-moves 480
    modal run cloud/modal/modal_app.py::corpus --backend python --agent ca_combo_v2 --opponent ca_combo_v2 \\
...
def corpus(
    ...
    out_path = Path(out) if out else RESULTS_ROOT / f"modal_corpus_{backend}_{n_games}.json"
...
def corpus_moves(
    ...
    print(json.dumps(summary, indent=2))
    out_path = Path(out) if out else RESULTS_ROOT / f"modal_moves_{backend}_{n_games}.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)

...
Usage (from hexo-theory/; Modal already authenticated as 'sub-surface'):
    ...
    modal run modal_rust_bot.py::smoke_test
    modal run modal_rust_bot.py::benchmark
    modal run modal_rust_bot.py::vs_mcts --games 60 --sims 100
    modal run modal_rust_bot.py::vs_sealbot --games 40
"""
...
def _play_vs_sealbot(seed: int, sealbot_time_limit: float, rust_time_budget: float,
    ...
    papers/misc/hexbot-building-framework/opponents/ramora/ai.py). ramora's
...
def benchmark(budgets: str = "0.3,0.7,1.5,3.0", n_trials: int = 6):
    ...
    out_path = THEORY_ROOT / "results" / "rust_search_benchmark.json"
...
def vs_mcts(games: int = 60, sims: int = 100, rust_time_budget: float = 0.70,
    ...
    out_path = Path(out) if out else THEORY_ROOT / "results" / "rust_vs_mcts100.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def vs_sealbot(games: int = 40, sealbot_time_limit: float = 0.70,
    ...
    out_path = Path(out) if out else THEORY_ROOT / "results" / "rust_vs_sealbot.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(

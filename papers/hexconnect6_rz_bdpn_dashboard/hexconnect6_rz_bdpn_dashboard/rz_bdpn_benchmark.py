...
def ap_flow_zone_n(env: HexEnv, black: int, white: int, n: int) -> Set[int]:
    rows = []
    for i in empty_cells(env, black, white):
        ...
        rows.append((score, i))
    rows.sort(reverse=True, key=lambda x: x[0])
    return {i for _, i in rows[:int(n)]}


def ap_flow_zone(env: HexEnv, black: int, white: int, cfg: Dict) -> Set[int]:
    return ap_flow_zone_n(env, black, white, cfg["zone_ap_cells"])
...
def branching_zone_n(env: HexEnv, black: int, white: int, reference_rows: List[Dict], n_moves: int) -> Set[int]:
...
def branching_zone(env: HexEnv, black: int, white: int, reference_rows: List[Dict], cfg: Dict) -> Set[int]:
    return branching_zone_n(env, black, white, reference_rows, cfg["zone_branch_moves"])


def branching_plus_ap_margin_zone(env: HexEnv, black: int, white: int, reference_rows: List[Dict], cfg: Dict) -> Set[int]:
    """
    Frontier zone: the high-precision branching proof core, plus a small AP-flow margin
    and the current obligation support. This is intended to sit between
    branching_debt and full combo on the recall/reduction frontier.
    """
    return (
        branching_zone_n(env, black, white, reference_rows, cfg["zone_margin_branch_moves"])
        | ap_flow_zone_n(env, black, white, cfg["zone_margin_ap_cells"])
...
def benchmark_position(env: HexEnv, pid: int, cfg: Dict):
    ...
    zones = {
        ...
        "branching_plus_ap_margin": branching_plus_ap_margin_zone(env, black, white, ref, cfg),
...
def run_benchmark(cfg: Dict):
    ...
    for pid in range(cfg["positions"]):
        if stop_file.exists():
            print(f"[stop] STOP file detected at {stop_file}; ending after completed checkpoints.", flush=True)
            break
        if pid in done:
            continue
        attempted += 1
    ...
    with (out / "benchmark_manifest.json").open("w", encoding="utf-8") as f:
...
```text
AP-flow ∪ obligation-support ∪ branching-debt; the smaller frontier test is branching_plus_ap_margin
```

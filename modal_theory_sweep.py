...
docs/theory/2026-07-08-pairing-thresholds-and-game-values.md:
  ...
  2. strip_entropy   -- the 2-axis-coupled "no 6-run on any of 3 HeXO axes"
     ...
     (1.965948..., matching results/line_automaton.json), and W=2 exactly
...
Usage (run from hexo-theory/; Modal already authenticated as 'sub-surface'):
    ...
    modal run modal_theory_sweep.py::smoke_test
    modal run modal_theory_sweep.py::pairing_sweep
    modal run modal_theory_sweep.py::strip_entropy_sweep --max-w 12
"""
...
def pairing_sweep(
    ...
    out_path = Path(out) if out else THEORY_ROOT / "results" / "pairing_capacity_phase_diagram.json"
...
def two_move_sum_sweep(lengths: str = "6,7,8,9", sizes: str = "2,3,4",
    ...
    print(json.dumps({k: v for k, v in summary.items()
                     ...
                     indent=2))
    out_path = Path(out) if out else THEORY_ROOT / "results" / "two_move_sum_full.json"
...
def residue_sweep(ks: str = "6,7", n_fronts_list: str = "20,40,60,80,120",
    ...
    out_path = Path(out) if out else THEORY_ROOT / "results" / "residue_defense_sweep.json"
...
def _strip_entropy_gpu(W: int, iters: int, seed_states: int) -> dict:
    ...
    Both W=1..4 are cross-checked exactly against results/strip_entropy.json
...
def strip_entropy_sweep(max_w: int = 12, iters: int = 300, seed_states: int = 60000, out: str = ""):
    ...
    out_path = Path(out) if out else THEORY_ROOT / "results" / "strip_entropy_gpu_sweep.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
